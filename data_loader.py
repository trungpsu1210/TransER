...
class SingleLoaderEMPatches(Dataset):
	def __init__(self, root_dir, patchsize = 2000, stride = 2000):
	...
	def __getitem__(self, idx):
		...
		emp = EMPatches()
		data_patches, indices = emp.extract_patches(img, patchsize=self.patchsize, stride=self.stride)
		for i in range(len(data_patches)):
			data_patches[i] = hwc_to_chw(data_patches[i])


		return {'patches': data_patches, 'indices': indices, 'filename': img_name}

class PairLoaderEMPatches(Dataset):
	def __init__(self, data_dir, sub_dir = 'test', patchsize = 2000, stride = 2000):
		self.patchsize = patchsize
		self.stride = stride
	...
	def __getitem__(self, idx):
		...
		emp = EMPatches()
		data_patches, indices = emp.extract_patches(source_img, patchsize=self.patchsize, stride=self.stride)
		for i in range(len(data_patches)):
			data_patches[i] = hwc_to_chw(data_patches[i])


		return {'patches': data_patches, 'indices': indices,
				'filename': img_name, 'target': hwc_to_chw(target_img)}
