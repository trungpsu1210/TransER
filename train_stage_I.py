...
def train(train_loader, network, criterion, optimizer, scaler):
    ...
    for batch in train_loader:
        ...
        with autocast(args.no_autocast):
            output, dehaze_reconstruct, x_J, x_A = network(source_img)
            loss_rec = criterion[0](output, target_img) + criterion[0](dehaze_reconstruct, target_img) + criterion[0](x_J, target_img)
            loss_ssim = - criterion[1](output, target_img) - criterion[1](dehaze_reconstruct, target_img) - criterion[1](x_J, target_img)
            loss_pec = criterion[2](output, target_img) + criterion[2](dehaze_reconstruct, target_img) + criterion[2](x_J, target_img)
            loss_varA = torch.std(x_A) ** 2

            loss = loss_rec + 0.0001 * loss_varA + 0.5 * loss_ssim + 0.1 * loss_pec
...
if __name__ == '__main__':
    ...
    if setting['optimizer'] == 'adam':
        optimizer = torch.optim.Adam(network.parameters(), lr=setting['lr_TFD'])
    elif setting['optimizer'] == 'adamw':
        optimizer = torch.optim.AdamW(network.parameters(), lr=setting['lr_TFD'])
    else:
    ...
    train_loader = DataLoader(train_dataset,
                              batch_size=setting['batch_size_Stage_I'],
                              shuffle=True,
    ...
    if args.resume and os.path.exists(os.path.join(save_dir, 'TFD.pth')):
        ckp = torch.load(os.path.join(save_dir, 'TFD.pth'))
    ...
    for epoch in range(start_epoch + 1, setting['epochs_TFD'] + 1):
